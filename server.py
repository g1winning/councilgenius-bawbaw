#!/usr/bin/env python3
"""
CouncilGenius V8 - Baw Baw Shire Council
...
COUNCIL_DOMAIN = "www.bawbawshire.vic.gov.au"
COUNCIL_PHONE = "(03) 5624 2411"
...
def classify(question):
    """
    V9 Classify - Multi-scoring category detection with expanded keywords.
    Returns highest-scoring category with semantic understanding.
    """
    ...
    categories = {
        'waste_bins': [
            'bin', 'bins', 'waste', 'rubbish', 'garbage', 'collection', 'recycling', 'recycle',
            'compost', 'refuse', 'pickup', 'kerbside', 'household waste', 'general waste',
            'food waste', 'green waste', 'landfill', 'wheelie bin', 'rubbbish collection'
        ],
        'rates': [
            'rates', 'council rates', 'rate', 'valuation', 'assessment', 'annual',
            'property tax', 'council tax', 'payment', 'bill', 'charges', 'fee',
            'amount owing', 'arrears', 'default', 'pay rates', 'rate notice'
        ],
        'planning': [
            'planning', 'development', 'permit', 'permission', 'application', 'approval', 'zoning',
            'building', 'construction', 'alteration', 'extension', 'renovation', 'approval process',
            'DA', 'development approval', 'building permit', 'planning scheme',
            'build', 'construct', 'deck', 'fence', 'pool', 'shed', 'carport', 'pergola',
            'granny flat', 'demolish', 'retaining wall', 'setback', 'dwelling', 'subdivision'
        ],
        'roads': [
            'road', 'street', 'pothole', 'repair', 'maintenance', 'footpath', 'pavement',
            'street light', 'drainage', 'flooding', 'damage', 'hazard', 'broken path',
            'infrastructure', 'street maintenance', 'pot hole', 'report', 'issue',
            'graffiti', 'vandalism', 'damaged', 'broken', 'safety', 'sign'
        ],
        'pets': [
            'pet', 'dog', 'cat', 'animal', 'registration', 'control', 'registration fee',
            'microchip', 'cat curfew', 'dog registration', 'pet permit', 'desex', 'de-sex'
        ],
        'property': [
            'property', 'land', 'title', 'boundary', 'land registration', 'property info',
            'owner', 'ownership', 'assessment', 'zoning'
        ],
        'family': [
            'family', 'child care', 'kindergarten', 'school', 'community', 'children',
            'early childhood', 'playgroup', 'children service'
        ],
        'community': [
            'community', 'event', 'service', 'local', 'volunteer', 'program', 'activity',
            'neighbourhood', 'group', 'support', 'library', 'libraries', 'swimming',
            'leisure', 'recreation', 'centre', 'center', 'hall', 'tourism', 'visitor',
            'museum', 'arts', 'culture', 'sport', 'venue', 'hire'
        ],
        'food_business': [
            'food', 'restaurant', 'cafe', 'business', 'registration', 'license', 'food safety',
            'food handling', 'catering', 'food business', 'food preparation'
        ],
        'contact': [
            'contact', 'phone', 'email', 'office', 'department', 'hours', 'when open',
            'address', 'location', 'get in touch', 'speak to',
            'emergency', 'after hours', 'urgent', 'open', 'closed', 'when', 'where'
        ],
        'libraries': [
            'library', 'libraries', 'books', 'borrow', 'reading', 'library hours', 'library location',
            'library services', 'members'
        ],
        'swimming': [
            'swimming', 'pool', 'swim', 'aquatic', 'water', 'leisure', 'facility',
            'pool opening hours', 'entry fee', 'entry fees'
        ],
        'environment': [
            'environment', 'sustainability', 'water', 'energy', 'green', 'sustainability',
            'waste reduction', 'recycling', 'composting'
        ],
        'legal': [
            'law', 'regulation', 'bylaw', 'legislation', 'legal', 'legal advice',
            'compliance', 'ordinance'
        ],
        'grants': [
            'grant', 'funding', 'subsidy', 'assistance', 'support', 'financial aid',
            'community grant'
        ],
        'local_laws': [
            'local laws', 'bylaws', 'ordinance', 'noise', 'parking', 'dog off-lead',
            'neighbourhood dispute'
        ],
        'forms': [
            'form', 'application', 'document', 'download', 'template'
        ]
    }
    ...
    for category, keywords in categories.items():
        ...
        for keyword in keywords:
            if keyword in question_lower:
                # Exact multi-word matches get higher weight
                if ' ' in keyword:
                    score += 3
                else:
                    score += 1
    ...
    # Return highest-scoring category, default to generic response
    if scores:
...
class CouncilGeniusHandler(BaseHTTPRequestHandler):
    ...
    def handle_chat(self):
        ...
        try:
            ...
            # Classify category using V9
            category = classify(user_message)

            # Check for abuse
            if any(word in user_message.lower() for word in ['hack', 'breach', 'attack', 'vulnerable', 'exploit']):
                ...
                log_query_basic(client_ip, filtered_question, 0, 'potential_api_abuse')
                return
